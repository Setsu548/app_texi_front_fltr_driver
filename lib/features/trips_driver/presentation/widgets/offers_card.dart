import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:go_router/go_router.dart';
import 'package:sizer/sizer.dart';
import 'package:texi_driver/core/lang/extension_lang.dart';
import 'package:texi_driver/core/router/app_router.dart';
import 'package:texi_driver/core/theme/styles_for_texts.dart';
import 'package:texi_driver/core/widgets/custom_snack_bar.dart';
import 'package:texi_driver/core/widgets/elevated_button_widget.dart';
import 'package:texi_driver/features/trips_driver/data/model/trip_offer_model.dart';
import 'package:texi_driver/features/trips_driver/presentation/providers/trip_offers_provider.dart';
import 'package:texi_driver/features/trips_driver/services/trip_states_services.dart';

class OffersCard extends ConsumerWidget {
  final TripOfferModel offer;
  const OffersCard({super.key, required this.offer});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final tripAcceptedInfoNotifier = ref.read(
      tripAcceptedInfoProvider.notifier,
    );
    return Card(
      color: Theme.of(context).colorScheme.secondary.withAlpha(50),
      margin: const EdgeInsets.symmetric(vertical: 8, horizontal: 16),
      elevation: 3,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      child: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
              'ID: ${offer.tripId}',
              style: StylesForTexts(context: context).bodyStyle().copyWith(
                color: Theme.of(
                  context,
                ).colorScheme.secondary.withValues(alpha: 0.5),
                fontSize: 14.85.sp,
              ),
            ),
            SizedBox(height: 0.55.h),
            SizedBox(
              width: 70.w,
              child: Text(
                '${passengerName.i18n}: ${offer.passengerName}',
                style: StylesForTexts(context: context).bodyStyle().copyWith(
                  color: Theme.of(context).colorScheme.primary,
                  fontSize: 15.85.sp,
                ),
              ),
            ),
            SizedBox(height: 0.55.h),
            SizedBox(
              width: 70.w,
              child: Text(
                '${estimatedTime.i18n}: ${offer.etaMinutes} min',
                style: StylesForTexts(context: context).bodyStyle().copyWith(
                  color: Theme.of(context).colorScheme.primary,
                  fontSize: 15.85.sp,
                ),
              ),
            ),
            SizedBox(height: 0.55.h),
            SizedBox(
              width: 70.w,
              child: Text(
                '${distanceToPickup.i18n}: ${offer.distanceToPickupKm.toStringAsFixed(2)} km',
                style: StylesForTexts(context: context).bodyStyle().copyWith(
                  color: Theme.of(context).colorScheme.primary,
                  fontSize: 15.85.sp,
                ),
              ),
            ),
            SizedBox(height: 0.55.h),
            SizedBox(
              width: 70.w,
              child: Text(
                '${tripDistance.i18n}: ${offer.tripDistanceKm.toStringAsFixed(2)} km',
                style: StylesForTexts(context: context).bodyStyle().copyWith(
                  color: Theme.of(context).colorScheme.primary,
                  fontSize: 15.85.sp,
                ),
              ),
            ),
            SizedBox(height: 0.55.h),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(
                  offeredPrice.i18n,
                  style: StylesForTexts(context: context).bodyStyle().copyWith(
                    color: Theme.of(context).colorScheme.primary,
                    fontSize: 15.85.sp,
                  ),
                ),
                Text(
                  '${offer.offeredPrice.toStringAsFixed(2)} Bs.',
                  style: StylesForTexts(context: context).headerStyleSmall(),
                ),
              ],
            ),
            SizedBox(height: 3.h),
            Row(
              mainAxisAlignment: MainAxisAlignment.end,
              children: [
                /* TextButton(
                  onPressed: () {
                    final offertRejectedId = offer.tripId;
                    ScaffoldMessenger.of(
                      context,
                    ).showSnackBar(customSnackBar(rejectedOffer.i18n, context));
                  },
                  child: Text(reject.i18n),
                ), */
                SizedBox(width: 3.w),
                ElevatedButtonWidget(
                  label: accept.i18n,
                  width: 28.w,
                  onPressed: () async {
                    final offertAcceptedId = offer.tripId;
                    if (await TripStatesServices.acceptTrip(
                      offertAcceptedId,
                      ref,
                    )) {
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          customSnackBar(acceptedOffer.i18n, context),
                        );
                        context.push(
                          '${AppRouter.tripPassengerOffersLocation}/${AppRouter.roadToOriginLocation}',
                          extra: offertAcceptedId,
                        );
                        tripAcceptedInfoNotifier.setTripAcceptedInfo(offer);
                      }
                    } else {
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          customSnackBar(errorAcceptOffer.i18n, context),
                        );
                      }
                    }
                  },
                ),
              ],
            ),
          ],
        ),
      ),
    );
  }
}
